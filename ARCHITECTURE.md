...
```
 ...
 └──────────────────┘          └─────────────────────┘          └───────────────────────────────┘
   (remote hosts (post-0.1.0) are just another source feeding the same model)
```
...
| `crates/collector` | Source implementations: `nvml`, `dcgm` (optional, richer/MIG), the inference scraper (post-0.1.0), and `mock` |
...
| `crates/app` | The single binary: subcommand dispatch (`gui`/`top`/`ps`; `serve` post-0.1.0), wires `collector → core → {ui｜cli｜sinks}` |
...
3. **Two first-class frontends, one binary.** GUI and CLI/TUI at parity, dispatched by subcommand
   (Docker/Ollama DX): `gui` (default) · `top` · `ps`/`ps --json` · `serve` (post-0.1.0). The CLI is a real
...
6. **Integrate, don't reimplement (the `Sink` seam).** Exporters — Prometheus, OTLP, Splunk (post-0.1.0) — are
...
- **Inference runtimes (post-0.1.0):** **Ollama** (`/api/ps`, the first integration), **vLLM / Triton / TGI**
  (Prometheus). Optional, auto-discovered or opt-in.
- **Exporters (post-0.1.0):** **Prometheus** `/metrics`, **OTLP** (→ Grafana/Datadog/Honeycomb/any OTel
...
- **Daemon mode is opt-in and additive — never the default.** agent is local-first and zero-config:
  ...
  exporter (`/metrics` alongside any mode) or the `serve` collector (the Ollama-style invisible
  daemon, managed by `systemd`/a container), both post-0.1.0 — is purely additive; disabling it leaves a fully useful
...
- **Non-goals (the anti-platform line).** agent is **not** a time-series database, a dashboard, or an
  alerting platform — it *exposes* data to the stack you already run (post-0.1.0). It is **not** a
